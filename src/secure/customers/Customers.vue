<template>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-sm">
        ...
        <tbody>
        <tr v-for="role in customers" :key="role.id">
          <td>{{ role.id }}</td>
          <td>{{ role.name }}</td>
          <td>
            <div class="btn-group mr-2">
              <router-link :to="`/customers/${role.id}/edit`" class="btn btn-sm btn-outline-secondary">Edit
              </router-link>
    ...
    setup() {
      ...
      const del = async (id: number) => {
        if (confirm('Are you sure you want to delete this record?')) {
          ...
          customers.value = customers.value.filter((e: Entity) => e.id !== id);
        }
      }
